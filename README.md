...
| 인자 | 필수 | 설명 |
|------|------|------|
| `pipeline\|fanout\|feedback\|hybrid\|p2p` | O | 실행 패턴 선택 |
| `--sub-agents` | - | 각 에이전트에 서브에이전트 스폰 기능 활성화 (teammateMode: in-process) |
| `<task-description>` | O | 작업 설명 (자연어) |

### 글로벌 옵션: `--sub-agents`

`--sub-agents`를 추가하면 spawn되는 모든 에이전트에 Claude Code의 Agent Teams 기능이 활성화됩니다. 각 에이전트는 자신의 작업을 더 작은 서브에이전트로 분할하여 **2단계 병렬화**를 달성합니다.
...
자세한 내용은 [서브에이전트 (teammateMode)](#서브에이전트-teammatemode) 섹션을 참조하세요.
...
# 서브에이전트 활성화 (각 에이전트가 내부적으로 더 작은 서브에이전트를 스폰)
> /cmux-agent-teams pipeline --sub-agents "User 도메인 API 개발: Entity 설계 → Service 구현 → Controller 작성"
```
...
`--sub-agents`를 추가하면 각 에이전트가 Claude Code의 Agent Teams 기능(teammateMode: in-process)으로 자신의 작업을 더 작은 서브에이전트로 분할하여 병렬 처리합니다. 복잡한 작업일수록 효과적입니다.
...
# 서브에이전트 활성화
> /cmux-agent-teams fanout --sub-agents "3개 독립 모듈 병렬 개발: 인증(Auth), 주문(Order), 알림(Notification)"
```
...
# --direction 생략 시 자동 레이아웃: Agent-A=위, Agent-B=오른쪽, 오케스트레이터=하단
...
│  │ (Claude #5)  │ (Claude #6)  │ (Claude #7)  │  (에이전트 4+) │
│  │  frontend-   │  test-       │  review-     │                │
│  │  api         │  writer      │  code        │                │
│  └──────┬───────┴──────┬───────┴──────┬───────┘                │
│         │              │              │                        │
│  ┌──────┴──────────────┴──────────────┴───────┐                │
│  │ Orchestrator (Main Claude) — 하단 고정      │                │
...
행당 최대 3개 에이전트. 4번째부터 자동으로 새 행 생성.
```
...
# 서브에이전트 활성화 (각 에이전트가 내부적으로 서브에이전트 스폰 가능)
> /cmux-agent-teams pipeline --sub-agents "User 도메인: Entity → Service → Controller 순차 개발"
```
...
# 서브에이전트 활성화
> /cmux-agent-teams fanout --sub-agents "3개 독립 모듈 병렬: 인증, 주문, 알림"
```
...
# 서브에이전트 활성화
> /cmux-agent-teams hybrid --sub-agents "프론트엔드 개발: 공통UI∥API훅 → 페이지 레이아웃"
```
...
# 서브에이전트 활성화 (풀스택 파이프라인은 작업이 복잡하므로 권장)
> /cmux-agent-teams hybrid --sub-agents "풀스택: API설계∥DB설계 → 백엔드 → 프론트 → 테스트∥리뷰"
```
...
# 서브에이전트 활성화
> /cmux-agent-teams hybrid --sub-agents "레거시 마이그레이션: 코드분석∥DB분석 → 계획 → 실행 → 검증"
```
...
# 서브에이전트 활성화
> /cmux-agent-teams p2p --sub-agents "CRUD API 구현(10 endpoints) + 즉시 테스트 작성"
```
...
| `--task` | O | - | 작업 설명 |
| `--direction` | - | 자동 | split 방향 수동 지정 (right/down/left/up). 생략 시 자동 레이아웃 |
| `--cwd` | - | `$PWD` | 작업 디렉터리 |
...
`--direction`을 생략하면 행당 최대 3개씩 그리드 배치됩니다. 오케스트레이터는 항상 하단 고정입니다:

```
┌─────────┬─────────┬─────────┐
│ Agent-1 │ Agent-2 │ Agent-3 │  ← row 0
├─────────┼─────────┼─────────┤
│ Agent-4 │ Agent-5 │ Agent-6 │  ← row 1 (4번째부터 자동 새 행)
├─────────┴─────────┴─────────┤
│      Orchestrator (하단)     │  ← 오케스트레이터 고정
└─────────────────────────────┘
```

- 첫 번째 에이전트: 오케스트레이터 위로(up) 분할
- 같은 행 추가(2~3번째): 이전 에이전트 오른쪽(right)으로 분할
- 새 행 시작(4, 7, 10...번째): 이전 행 첫 에이전트 아래(down)로 분할
...
# --sub-agents 플래그 추가 → 모든 에이전트에 서브에이전트 활성화
> /cmux-agent-teams pipeline --sub-agents "User 도메인 API 개발: Entity 설계 → Service 구현 → Controller 작성"

# 모든 패턴에서 사용 가능
> /cmux-agent-teams fanout --sub-agents "3개 독립 모듈 병렬: 인증, 주문, 알림"
> /cmux-agent-teams hybrid --sub-agents "풀스택: API설계∥DB설계 → 백엔드 → 프론트"
> /cmux-agent-teams p2p --sub-agents "CRUD API + 테스트 병렬"
```

`--sub-agents`를 스킬에 전달하면 내부적으로 `spawn-agent.sh`에 `--sub-agents` 플래그가 전파되어, 생성되는 모든 에이전트에 teammateMode 설정이 적용됩니다.
